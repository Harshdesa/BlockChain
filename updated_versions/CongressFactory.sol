contract CongressFactory {
    address[] congressContracts;
    mapping (address => address[]) public contracts;

    event NewContract(address newContract);
        
    function CongressFactory() public {
    }
    ...
    function createCongress(uint minimumQuorumForProposals, uint minutesForDebate, int marginOfVotesForMajority) public payable returns (address _congressAddress) {
        address congressContract = new Congress(minimumQuorumForProposals, minutesForDebate, marginOfVotesForMajority);
        congressContracts.push(congressContract);
        contracts[msg.sender].push(congressContract);
...
contract Congress is owned, tokenRecipient {
    ...
    function getProposalDecision(uint proposalNumber) public returns (uint decision_code) {
        ...
        else if (!p.executed && p.numberOfVotes >= minimumQuorum && p.currentResult > majorityMargin) {
            return 1;
        }
        else {
            return 0;
